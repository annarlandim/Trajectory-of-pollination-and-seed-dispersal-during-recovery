...
jagsModel <- 
  ...
  for(j in 1:n_var) {
    # Priors diretos para cada grupo (SEM hierarquia)
    theta_inf[j] ~ dlnorm(0, 1)   # ou dlnorm(μ, τ) ajustado ao que seu colaborador preferir
  ...
  for(j in 1:n_var) {
    alpha_con[j] ~ dnorm(0, 1)
    beta_con[j]  ~ dnorm(0, 1)
  }
  ...
  for (i in 1:n_old) {
    Y_old[i] ~ dlnorm( log(theta_inf[variable_old[i]]),
                       tau_old[variable_old[i]] )
  }
  ...
  for (i in 1:n_rec) {
    ...
    Y_rec[i] ~ dlnorm( log(mu_rec[i]),
                       tau_rec[variable_rec[i]] )
  }
}"
cat(jagsModel, file = "jagsModel.txt")
