...
net_df_plants_rich_z <- data %>%
  select(all_of(all_groups), all_of(plant_rich)) %>%
  mutate(across(everything(), ~ scale(log(. + 1))))
...
df_resids_plants_rich_str <- data %>% select(all_of(c(all_groups, plant_rich)), "StrIndex") %>%
  mutate(across(all_of(c(all_groups, plant_rich)), ~ log(. + 1)))

net_resids_df_plants_rich_str <- as.data.frame(lapply(c(all_groups, plant_rich), get_resids_str, data = df_resids_plants_rich_str)) %>%
  mutate(across(everything(), ~ as.numeric(scale(.)))) # to make sure residuals are in the same scale, so comparable

colnames(net_resids_df_plants_rich_str) <- c(all_groups, plant_rich)
...
target_vars <- c(all_groups, plant_rich)
control_vars <- c("StrIndex")
...
df_resids_plants_rich_con <- data %>% select(all_of(c(all_groups, plant_rich)), "ConIndex") %>%
  mutate(across(all_of(c(all_groups, plant_rich)), ~ log(. + 1)))

net_resids_df_plants_rich_con <- as.data.frame(lapply(c(all_groups, plant_rich), get_resids_con, data = df_resids_plants_rich_con)) %>%
  mutate(across(everything(), ~ as.numeric(scale(.)))) # to make sure residuals are in the same scale, so comparable

colnames(net_resids_df_plants_rich_con) <- c(all_groups, plant_rich)
...
df_resids_plants_rich_both <- data %>% select(all_of(c(all_groups, plant_rich)), "StrIndex", "ConIndex") %>%
  mutate(across(all_of(c(all_groups, plant_rich)), ~ log(. + 1)))

net_resids_df_plants_rich_both <- as.data.frame(lapply(c(all_groups, plant_rich), get_resids_both, data = df_resids_plants_rich_both)) %>%
  mutate(across(everything(), ~ as.numeric(scale(.)))) # to make sure residuals are in the same scale, so comparable

colnames(net_resids_df_plants_rich_both) <- c(all_groups, plant_rich)
...
df_resids_plants_shn_both <- data %>% select(all_of(c(all_groups, plant_shn)), "StrIndex", "ConIndex") %>%
  mutate(across(all_of(c(all_groups, plant_shn)), ~ log(. + 1)))

net_resids_df_plants_shn_both <- as.data.frame(lapply(c(all_groups, plant_shn), get_resids_both, data = df_resids_plants_shn_both)) %>%
  mutate(across(everything(), ~ as.numeric(scale(.)))) # to make sure residuals are in the same scale, so comparable

colnames(net_resids_df_plants_shn_both) <- c(all_groups, plant_shn)
...
df_resids_plants_fd_both <- data %>% select(all_of(c(all_groups, plant_fd)), "StrIndex", "ConIndex") %>%
  mutate(across(all_of(c(all_groups, plant_fd)), ~ log(. + 1)))

net_resids_df_plants_fd_both <- as.data.frame(lapply(c(all_groups, plant_fd), get_resids_both, data = df_resids_plants_fd_both)) %>%
  mutate(across(everything(), ~ as.numeric(scale(.)))) # to make sure residuals are in the same scale, so comparable

colnames(net_resids_df_plants_fd_both) <- c(all_groups, plant_fd)
...
results_nets <- list(
  processes = list(
    crude = list(net = cor_matrix_all, sig = edge_sig_all, ci = edge_ci_all),
    str   = list(net = cor_matrix_res_str, sig = edge_sig_all_ctrl_str, ci = edge_ci_all_ctrl_str),
    con   = list(net = cor_matrix_res_con, sig = edge_sig_all_ctrl_con, ci = edge_ci_all_ctrl_con),
    both  = list(net = cor_matrix_res_both, sig = edge_sig_all_ctrl_both, ci = edge_ci_all_ctrl_both)
  ),
  plants_ab = list(
    crude = list(net = cor_matrix_plants_ab, sig = edge_sig_plants_ab, ci = edge_ci_plants_ab),
    str   = list(net = cor_matrix_res_plants_ab_str, sig = edge_sig_plants_ab_ctrl_str, ci = edge_ci_plants_ab_ctrl_str),
    con   = list(net = cor_matrix_res_plants_ab_con, sig = edge_sig_plants_ab_ctrl_con, ci = edge_ci_plants_ab_ctrl_con),
    both  = list(net = cor_matrix_res_plants_ab_both, sig = edge_sig_plants_ab_ctrl_both, ci = edge_ci_plants_ab_ctrl_both)
  ),
  plants_rich = list(
    crude = list(net = cor_matrix_plants_rich, sig = edge_sig_plants_rich, ci = edge_ci_plants_rich),
    str   = list(net = cor_matrix_res_plants_rich_str, sig = edge_sig_plants_rich_ctrl_str, ci = edge_ci_plants_rich_ctrl_str),
    con   = list(net = cor_matrix_res_plants_rich_con, sig = edge_sig_plants_rich_ctrl_con, ci = edge_ci_plants_rich_ctrl_con),
    both  = list(net = cor_matrix_res_plants_rich_both, sig = edge_sig_plants_rich_ctrl_both, ci = edge_ci_plants_rich_ctrl_both)
  ),
  plants_shn = list(
    ...
    both  = list(net = cor_matrix_res_plants_shn_both, sig = edge_sig_plants_shn_ctrl_both, ci = edge_ci_plants_shn_ctrl_both)
  ),
  plants_fd = list(
    ...
    both  = list(net = cor_matrix_res_plants_fd_both, sig = edge_sig_plants_fd_ctrl_both, ci = edge_ci_plants_fd_ctrl_both)
  )
)
